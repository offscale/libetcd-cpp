...
mkdir -p etcd/mvcc/mvccpb
"${CMD[@]}" --cpp_out=etcd/mvcc/mvccpb "$REPO_ROOT"/mvcc/mvccpb/*.proto

mkdir -p etcd/auth/authpb
"${CMD[@]}" --cpp_out=etcd/auth/authpb "$REPO_ROOT"/auth/authpb/*.proto
